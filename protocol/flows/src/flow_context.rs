...
impl FlowContext {
    ...
    pub async fn register_hash_for_processing_loop(&self, peer: Option<PeerKey>, hash: Hash) -> bool {
        if let Some(peer_key) = peer {
            self.block_inv_processing_metadata.lock().insert_hash(peer_key, hash)
        } else {
            self.block_inv_processing_metadata
                .lock()
                .insert_hash(PeerKey::new(PeerId::default(), IpAddress::new(IpAddr::V4(Ipv4Addr::UNSPECIFIED)), 0), hash)
        }
    }
